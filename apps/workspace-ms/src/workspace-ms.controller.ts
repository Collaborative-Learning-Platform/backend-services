...
export class WorkspaceMsController {
  ...
  async addUsersToWorkspace(
    ...
    data: {
      workspaceId: string;
      fileData: { originalname: string; mimetype: string; buffer: any };
    },
  ) {
    // console.log(data)
    ...
    if (Buffer.isBuffer(fileData.buffer)) buffer = fileData.buffer;
    else if (
  ...
  async getUserGroupsInWorkspace(
    @Payload() data: { userId: string; workspaceId: string },
  ) {
    console.log(
      'Received get user groups in workspace request at microservice:',
      data,
    );
    return this.workspaceMsService.getUserGroupsInWorkspace(data);
  }

  //get workspace, and user joined groups inside that together
  @MessagePattern({ cmd: 'get_user_workspaces_with_groups' })
  async getUserWorkspacesWithGroups(@Payload() data: { userId: string }) {
    return this.workspaceMsService.getUserWorkspacesWithGroups(data);
  }

  //get all groups a user has joined
  @MessagePattern({ cmd: 'get_groups_by_user' })
  async getGroupsByUser(@Payload() data: { userId: string }) {
