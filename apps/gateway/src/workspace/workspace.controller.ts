...
//workspace APIs
 ...
 //Group APIs
    ...
    async createCustomGroup(@Req() req: any, @Res() res: Response) {
        ...
        const response = await lastValueFrom(this.WorkspaceClient.send({ cmd: 'create_custom_group' }, data));
        if (response?.error) {
            const ret = handleValidationError(response.error);
            return res.json(ret);
        }
    ...
    async fetchGroupDetails(@Param('groupId') groupId: string, @Res() res: Response) {
        console.log('Received fetch group details request at gateway:', groupId);
        const response = await lastValueFrom(this.WorkspaceClient.send({ cmd: 'get_group_details' }, { groupId }));
        if (response?.error) {
            const ret = handleValidationError(response.error);
            return res.json(ret);
        }
    ...
    async fetchGroupUsers(@Param('groupId') groupId: string, @Res() res: Response) {
        console.log('Received fetch group users request at gateway:', groupId);
        const response = await lastValueFrom(this.WorkspaceClient.send({ cmd: 'get_group_users' }, { groupId }));
        if (response?.error) {
            const ret = handleValidationError(response.error);
            return res.json(ret);
        }
    ...
    async addUsersToGroup (@Param('groupId') groupId: string, @Body() body: { members: string[] }, @Res() res: Response) {
        const data = {
            groupId,
            userIds: body.members,
        };
        console.log('Received add users to group request at gateway:', data);
        const response = await lastValueFrom(this.WorkspaceClient.send({ cmd: 'add_users_to_group' }, data));
        if (response?.error) {
            const ret = handleValidationError(response.error);
            return res.json(ret);
        }
